...
- 当前工具执行边界：`loadSkill` 能返回内置/线上 skill instructions；`run_intent(send_email)` 会拉起 Android 邮件 Intent；`run_js` 因 JS/WebView sandbox 尚未接通，返回 `pending_bridge`，明确要求 Gemma 不得伪装执行成功。
