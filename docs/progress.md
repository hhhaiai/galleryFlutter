...
- [x] Android Skills 工具桥接第一阶段：Skills runtime 启用 `ToolProvider` / constrained decoding；支持 `loadSkill` 返回内置/线上 skill instructions；支持 `run_intent(send_email)` 拉起邮件 Intent；已把 Gallery built-in skill assets 打包进 Android 并用本地 headless WebView 执行 bundled `run_js`
...
- [ ] Skills 的 JS/WebView sandbox 深化：线上/custom skill JS 文件下载执行、结果图片/WebView 原生展示、secret/API key 管理
